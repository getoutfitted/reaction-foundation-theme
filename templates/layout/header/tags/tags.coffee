...
Template.foundationHeaderTags.helpers
  tagsComponent: ->
    ...
    if isEditing(currentTag()) and ReactionCore.hasOwnerAccess()
      return Template.foundationTagInputForm
    else
      return Template.foundationHeaderLinks
...
Template.foundationHeaderLinks.helpers
  activeTag: ->
    return "active" if Session.equals "currentTag", @._id

Template.foundationTagInputForm.helpers
  tags: ->
    tagList = []
    for tag in @.tags
...
Template.foundationHeaderLinks.events
  'click #header-edit-tag': (event,template) ->
    setEditing currentTag(), true
    Tracker.flush()

Template.foundationTagInputForm.events
...
Template.foundationTagInputForm.rendered = ->
  # *****************************************************
